...
| Shipping | Continental U.S. only (48 states + D.C.), configurable methods + free-shipping threshold (default $250). Methods can be flat-rate or live carrier rates (via Shippo) calculated per order from the ship-from address, package weight, and destination — never trusted from the client. For live-carrier methods, admins can one-click "Buy Shipping Label" on an order (Shippo purchases the label, auto-fills tracking number/carrier/URL, provides a printable PDF, marks the order Shipped, and sends the shipping-confirmation email) instead of buying a label elsewhere and typing tracking in by hand |
...
| Address autocomplete | Checkout's street-address field offers a typeahead dropdown (Smarty) that prefills line 2/city/state/ZIP on selection — optional (`SMARTY_AUTH_ID`/`SMARTY_AUTH_TOKEN`), falls back to plain manual entry if unset; results are restricted to states the site actually ships to |
